...
public class MainActivity extends Activity implements OnCompletionListener, SeekBar.OnSeekBarChangeListener {
    ...
    public LinkedList<File> songsList = new LinkedList<File>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.songsList = songManager.getFilesInFolder(extStore);
        Log.i(null, ""+songsList.size());
        
        if(songsList.size() == 0 ) 
        { Toast.makeText(this, "NO FILE WAS FOUND", Toast.LENGTH_LONG).show();
          Log.e("mp", "not found");
          flag=1;
        }
        else
    ...
    public void  playSong(int songIndex){
        ...
        try {
            ...
            String songTitle = songsList.get(songIndex).getName().substring(0, ((songsList.get(songIndex)).getName().length() - 4));
            ...
            Bitmap bm=null;
            try {
				Mp3File mp3file = new Mp3File(songsList.get(songIndex).getAbsolutePath().toString(),false);
				if(mp3file.hasId3v2Tag() && mp3file.getId3v2Tag().getAlbumImage() != null)
				{
					...
					bm= bm.createScaledBitmap(bm, 350, 320, false);
				}
